...
async function renderJournalTable(filteredTrades = trades) {    
    ...
    for (const trade of filteredTrades) {    
        ...
        const screenshotIds = Array.isArray(trade.screenshotIds)
            ? trade.screenshotIds
        ...
        if (screenshotIds.length === 0) {
            ...
            for (let i = 0; i < screenshotIds.length; i++) {
                try {
                    ...
                    if (blob) {
                        ...
                        thumbUrls.push(url);
                        previewUrls.push(url); // store for preview page
                    }
                } catch (error) {
            ...
            const thumbs = thumbUrls.slice(0,4).map((url,i)=>`
                <img class="thumb"
                     data-index="${i}"
                     src="${url}"
                     style="width:48px;height:32px;object-fit:cover;border-radius:4px;margin-right:6px;cursor:pointer;border:1px solid #334155;">
            `).join('');

            const moreBadge = screenshotIds.length > 4
                ? `<span style="margin-left:6px;color:#94a3b8;font-size:0.85rem">+${screenshotIds.length-4}</span>`
                : '';
        ...
        row.innerHTML = `    
            ...
            <td>
                <span class="status-badge status-${pnlStatus.toLowerCase().replace(' ','-')}">
                    ${pnlStatus}
                </span>
            </td>    
            <td>${screenshotCellHtml}</td>    
            <td>${trade.mistakes}</td>    
            <td title="${trade.lessonsLearned}">
                ${lessonsTruncated}
            </td>    
            <td>${trade.psychology}</td>    
            <td class="action-cell"></td>    
            <td class="preview-cell" style="cursor:pointer;">P</td>
        `;
        ...
        previewCell.addEventListener('click', ()=>{
            ...
            const previewData = {
                ...trade,
                screenshotUrls: previewUrls   // send URLs instead of IDs
            };
            ...
            window.location.href = "table-detailed.html";
        });
        ...
        deleteBtn.addEventListener('click', async (e)=>{    
            ...
            if (!confirm("Are you sure you want to delete this trade and all its screenshots?"))
                return;
...
async function openModal(mode, tradeId = null) {
    ...
    // Load options first to ensure selects are populated
    await loadOptionsFromLocalStorage();
...
// በ loop ውስጥ እንዲህ መጠቀም ትችላለህ
Object.entries(fieldMap).forEach(([id, prop]) => {
    ...
    setTimeout(() => {
        el.value = trade[prop];
							...
							// Alternative approach - rebuild the select with the selected value
...
// Use it in your code:
    ...
    setTimeout(() => {
        ...
        // Handle screenshot label
        const screenshotIds = Array.isArray(trade.screenshotIds) ? trade.screenshotIds : [];
        fileLabel.textContent = screenshotIds.length > 0 ? `Has ${screenshotIds.length} image(s)` : "No file chosen";
    }
